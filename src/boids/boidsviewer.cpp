...
struct BoidsViewer : Viewer {
	...
	void render3D(const RenderApi3D& api) const override {
		...
		api.solidCube(cubeSize, boidsWhite, &cubeModelMatrix);

		{
			...
			api.lines(vertices, COUNTOF(vertices), boidsWhite, &cubeModelMatrix);
		}

		{
			glm::quat q = glm::angleAxis(boneAngle, glm::vec3(0.f, 1.f, 0.f));
			glm::vec3 childRelPos = { 1.f, 1.f, 0.f };
			api.bone(childRelPos, boidsWhite, q, glm::vec3(0.f, 0.f, 0.f));
			glm::vec3 childAbsPos = q * childRelPos;
			api.solidSphere(childAbsPos, 0.05f, 10, 10, boidsWhite);
		}

		api.solidSphere(glm::vec3(-1.f, 0.5f, 1.f), 0.5f, 100, 100, boidsRed);
	}
